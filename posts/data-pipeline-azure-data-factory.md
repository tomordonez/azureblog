...
Browsing through Data.gov and the DOT website, I found interesting data about `DBE` that I learned it means `Disadvantaged Business Enteprise`. After a bit more research I learned that there is a DBE website for every US state.

I found that the US Department of Transportation has a website with all US states and their corresponding DBE websites [here](https://www.transportation.gov/osdbu/disadvantaged-business-enterprise/state-dot-and-disadvantaged-business-enterprise-web-pages). However, most of those links didn't work.

After some googling, I learned that using the google query `unified certification program`, leads you to most states DBE websites. Many of them use the same platform that lets you export the data.
